...
public class AccelePayApplication implements CommandLineRunner {
    ...
    public void run(String... args) {
        var individual = User.builder().fullName("George Agboghoroma").apiKey(encrypt.encrypt("we love apis")).phoneNumber("08052267781").email("olamiretj@gmail.com").userType(UserType.PERSONAL).build();
        ...
        var merchant = User.builder().fullName("Adora's Skin Care").apiKey(encrypt.encrypt("skin care is health care")).phoneNumber("08052267789").email("ad_professional@gmail.com").userType(UserType.MERCHANT).build();
